...
export class GitHubAdmin {
  ...
  hasGitHubAccess() {
    ...
    if (!hasAccess) {
      console.log('⚠️  GitHub access not available:', {
  ...
  async updatePostStatus(slug, status) {
    ...
    try {
      ...
      if (!fileResponse.ok) {
        ...
        throw new Error(`Failed to fetch file: ${errorData.message || 'File not found'}`);
      }
      ...
      // Try different patterns for status field
      if (updatedContent.includes('status:')) {
        updatedContent = updatedContent.replace(
          /status:\s*["'][^"']*["']/g,
          `status: "${status}"`
        );
      } else {
        // If no status field exists, add it after the front matter
        ...
        if (frontMatterEnd !== -1) {
          ...
          updatedContent = beforeFrontMatter + `status: "${status}"\n` + afterFrontMatter;
        }
      }
