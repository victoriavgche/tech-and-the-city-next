...
export async function getPublishedPosts(){
  try {
    ...
    return allPosts.filter(post => !post.status || post.status !== 'draft');
  } catch (error) {
...
export async function getPostBySlug(slug){
  try {
    // Try database approach first (for production)
    try {
      ...
      if (post) {
        ...
        if (isHTML) {
          ...
          try {
            ...
            // Fallback: wrap content in paragraph tags
      ...
      console.error('Database approach failed, trying file system:', dbError);
    }
    
    // Fallback to file system approach (for development)
