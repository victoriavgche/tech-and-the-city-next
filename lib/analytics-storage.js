...
// Generate user fingerprint for unique user tracking
function generateUserFingerprint(userAgent, screen, viewport, timestamp) {
  const fingerprint = {
    userAgent: userAgent,
    screen: screen,
    viewport: viewport,
...
export function trackUniqueUserActivity(pageView) {
  const data = loadAnalyticsData();
  ...
  if (!data.uniqueUsers[userFingerprint]) {
    data.uniqueUsers[userFingerprint] = {
      firstSeen: today,
      lastSeen: today,
      totalViews: 0,
      pages: {},
    ...
    data.metadata.totalUniqueUsers++;
  }
  
  // Update user activity
  const user = data.uniqueUsers[userFingerprint];
  user.lastSeen = today;
  user.totalViews++;
  user.sessions.add(pageView.sessionId);
  user.pages[pageView.page] = (user.pages[pageView.page] || 0) + 1;
  
  // Track daily activity
  if (!data.userActivity[today]) {
    data.userActivity[today] = {
      uniqueUsers: new Set(),
      totalViews: 0,
      pageViews: {}
    };
  ...
  data.userActivity[today].uniqueUsers.add(userFingerprint);
  data.userActivity[today].totalViews++;
  data.userActivity[today].pageViews[pageView.page] = 
    (data.userActivity[today].pageViews[pageView.page] || 0) + 1;
  
  saveAnalyticsData(data);
}

// Get enhanced analytics summary with unique user data
export function getEnhancedAnalyticsSummary() {
  const data = loadAnalyticsData();
  ...
  // Get unique sessions
  const uniqueSessions = new Set(data.pageViews.map(view => view.sessionId)).size;
  
  // Get unique users
  const totalUniqueUsers = data.metadata.totalUniqueUsers;
  
  // Get page view breakdown (by unique users)
  const pageViewBreakdown = {};
  Object.values(data.uniqueUsers).forEach(user => {
    Object.entries(user.pages).forEach(([page, views]) => {
      pageViewBreakdown[page] = (pageViewBreakdown[page] || 0) + views;
    });
  });
  ...
  data.clicks.forEach(click => {
    clickBreakdown[click.element] = (clickBreakdown[click.element] || 0) + 1;
  });
  ...
  Object.values(data.uniqueUsers).forEach(user => {
    ...
    const range = views <= 1 ? '1 view' : 
                  views <= 5 ? '2-5 views' : 
                  views <= 10 ? '6-10 views' : 
                  views <= 20 ? '11-20 views' : '20+ views';
    userActivityBreakdown[range] = (userActivityBreakdown[range] || 0) + 1;
  });
