...
export function loadEventsData() {
  try {
    if (fs.existsSync(EVENTS_FILE)) {
      ...
      // Automatically set status based on date for all events
      return events.map(event => ({
        ...event,
        status: getEventStatusFromDate(event.date)
      }));
    }
  } catch (error) {
...
export function updateEvent(id, eventData) {
  ...
  if (eventData.isDraft !== undefined) {
    updatedEventData.status = eventData.isDraft ? 'draft' : getEventStatusFromDate(eventData.date);
